...
function codegen.gen_expression(node, ctx)
    ...
    if t == N.IDENTIFIER then
		...
		if node.name == "super" then
			-- Check if user defined a variable named 'super' first
			return '(__ctx["super"] ~= nil and __ctx["super"] or (__super or function() return \'\' end))'
		end
        ...
        for i, entry in ipairs(node.entries) do
            if entry.key then
				...
				if entry.value.type == N.TABLE then
					...
					value = nested_is_array and ("__runtime.list(" .. value .. ")")
						or ("__runtime.dict(" .. value .. ")")
				end
...
function codegen.gen_node(node, ctx)
	...
	if t == N.FILTER_BLOCK then
		...
		emit(
			ctx,
			'local __filter_result = __filters["' .. node.filter_name .. '"](' .. table.concat(filter_args, ", ") .. ")"
		)
    ...
    if t == N.INTERPOLATION then
		...
		local col = (node.expression and node.expression.type == N.LITERAL and node.expression.literal_type == "string")
				and 1
			or (node.column or 1)
	...
	if node.parent_block then
		...
		-- If parent block also has a parent (grandparent), set up its super function recursively
		if node.parent_block.parent_block then
			emit(ctx, "local __parent_prev_super = __super")
			emit(ctx, "__super = function()")
			indent(ctx)
			emit(ctx, "local __out = {}") -- Use __out in nested scope

			-- Render grandparent block content
			for _, child in ipairs(node.parent_block.parent_block.body) do
				codegen.gen_node(child, ctx)
			end
		...
		-- Restore grandparent super if needed
		if node.parent_block.parent_block then
			emit(ctx, "__super = __parent_prev_super")
		end
...
function codegen.gen_for(node, ctx)
    ...
    if #var_names > 1 then
		...
		emit(
			ctx,
			"for _ in pairs(" .. loop_var .. "_items) do " .. loop_var .. "_count = " .. loop_var .. "_count + 1 end"
		)
		...
		emit(
			ctx,
			'__ctx["loop"] = __runtime.context.loop_meta('
				.. loop_var
				.. "_idx, "
				.. loop_var
				.. "_count, nil, "
...
function codegen.gen_macro_call(node, ctx)
	...
	if node.name == "caller" and not node.caller_body then
		...
		emit(
			ctx,
			'__out[#__out + 1] = tostring(__ctx["caller"] and __ctx["caller"]('
				.. table.concat(args, ", ")
				.. ') or "")'
		)
		return
