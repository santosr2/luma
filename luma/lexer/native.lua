...
function native:scan_expression_token()
	...
	if c == "|" then
		return self:make_token(T.PIPE, nil, start_line, start_col)
	end
	if c == "." then
		return self:make_token(T.DOT, nil, start_line, start_col)
	end
	if c == "," then
		return self:make_token(T.COMMA, nil, start_line, start_col)
	end
	if c == ":" then
		return self:make_token(T.COLON, nil, start_line, start_col)
	end
	if c == "[" then
		return self:make_token(T.LBRACKET, nil, start_line, start_col)
	end
	if c == "]" then
		return self:make_token(T.RBRACKET, nil, start_line, start_col)
	end
	if c == "(" then
		return self:make_token(T.LPAREN, nil, start_line, start_col)
	end
	if c == ")" then
		return self:make_token(T.RPAREN, nil, start_line, start_col)
	end
	if c == "{" then
		return self:make_token(T.LBRACE, nil, start_line, start_col)
	end
	...
	if c == "<" then
		return self:make_token(T.LT, nil, start_line, start_col)
	end
	if c == ">" then
		return self:make_token(T.GT, nil, start_line, start_col)
	end
	if c == "+" then
		return self:make_token(T.PLUS, nil, start_line, start_col)
	end
	if c == "-" then
		return self:make_token(T.MINUS, nil, start_line, start_col)
	end
	if c == "*" then
		return self:make_token(T.STAR, nil, start_line, start_col)
	end
	if c == "/" then
		return self:make_token(T.SLASH, nil, start_line, start_col)
	end
	if c == "%" then
		return self:make_token(T.PERCENT, nil, start_line, start_col)
	end
	if c == "^" then
		return self:make_token(T.CARET, nil, start_line, start_col)
	end
	if c == "=" then
		return self:make_token(T.ASSIGN, nil, start_line, start_col)
	end
	if c == "#" then
		return self:make_token(T.HASH, nil, start_line, start_col)
	end
...
function native:scan_directive()
	...
	if
		dir_type == T.DIR_ELSE
		or dir_type == T.DIR_END
		or dir_type == T.DIR_RAW
		or dir_type == T.DIR_ENDRAW
		or dir_type == T.DIR_ENDSET
		or dir_type == T.DIR_ENDAUTOESCAPE
		or dir_type == T.DIR_ENDCALL
		or dir_type == T.DIR_ENDWITH
		or dir_type == T.DIR_ENDFILTER
		or dir_type == T.DIR_BREAK
		or dir_type == T.DIR_CONTINUE
	then
...
function native:scan_text()
	...
	while not self:is_eof() do
		...
		-- Only treat as trim directive if preceded by whitespace
		-- Otherwise the hyphen is literal text (e.g., ${a}-${b})
		if c == "-" then
...
function native:next_token()
	...
	if c == "-" and self:peek(1) == "@" then
		...
		if next_next and is_alpha(next_next) then
			...
			if self.at_line_start then
				...
				if
					prev_pos > 0
					and (self.source:sub(prev_pos, prev_pos) == " " or self.source:sub(prev_pos, prev_pos) == "\t")
				then
	...
	if c == "-" and self:peek(1) == "$" then -- luacheck: ignore (empty branch intentional)
		...
		-- scan_text will determine if it's a trim directive based on context
	end
