...
tokens.types = {
	...
	DIR_WITH = "DIR_WITH", -- @with (scoped variables)
	DIR_ENDWITH = "DIR_ENDWITH", -- @endwith
	DIR_FILTER = "DIR_FILTER", -- @filter name (apply filter to block)
	DIR_ENDFILTER = "DIR_ENDFILTER", -- @endfilter
	DIR_DO = "DIR_DO", -- @do expression (side effect)
...
tokens.directives = {
	...
	["with"] = tokens.types.DIR_WITH,
	["endwith"] = tokens.types.DIR_ENDWITH,
	["filter"] = tokens.types.DIR_FILTER,
	["endfilter"] = tokens.types.DIR_ENDFILTER,
	["do"] = tokens.types.DIR_DO,
...
function tokens.format(token)
	...
	return string.format("[%s%s] at %d:%d", tokens.type_name(token.type), value_str, token.line or 0, token.column or 0)
end
