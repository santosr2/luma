...
function expressions.parse_primary(stream)
	...
	errors.raise(errors.parse("Unexpected token: " .. tokens.type_name(token.type), token.line, token.column))
end
...
function expressions.parse_ternary(stream)
	...
	if stream:check(T.IF) then
		...
		if not stream:check(T.ELSE) then
			errors.raise(
				errors.parse("Expected 'else' in ternary expression", stream:peek().line, stream:peek().column)
			)
		end
